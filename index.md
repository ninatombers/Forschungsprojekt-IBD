...
<details><summary markdown="span">Click here for 'genotyping.sh' Script</summary>
...
INPUT_META=$BASE_DIR/metadata_ibd_complete
...
bwa mem -M -t 8 -p $BASE_DIR/data/Hpue_genome_unmasked_01.fas $BASE_DIR/outputs/2_align/1_fatsq/\${sample}.adapterfq > $BASE_DIR/outputs/2_align/2_align/\${sample}.aligned.sam
...
gatk --java-options "-Xmx68G" \
    ...
    -ALIGNED_BAM $BASE_DIR/outputs/2_align/2_align/\${sample}.aligned.sam \
    -UNMAPPED_BAM $BASE_DIR/outputs/0_ubam/\${sample}.ubam.bam \
    -OUTPUT $BASE_DIR/outputs/2_align/3_merge/\${sample}.mapped.bam \
    --REFERENCE_SEQUENCE $BASE_DIR/data/Hpue_genome_unmasked_01.fas.gz \
    -PAIRED_RUN true \
    --SORT_ORDER "unsorted" \
...
gatk --java-options "-Xmx30G" \
		SortSam \
		-I $BASE_DIR/outputs/2_align/3_merge/\${sample}.mapped.bam \
		-O /dev/stdout \
...
INPUT_COV=$BASE_DIR/outputs/lof/4_duplicates.fofn
...
gatk --java-options "-Xmx30G" \
  ...
  -R $BASE_DIR/data/Hpue_genome_unmasked_01.fas.gz
...
gatk --java-options "-Xmx35g" HaplotypeCaller  \
     -R $BASE_DIR/data/Hpue_genome_unmasked_01.fas\
...
sed -e 's/^/-V /' $BASE_DIR/outputs/lof/4_likelihood.fofn > $BASE_DIR/outputs/lof/4_likelihood2.fofn
...
gatk --java-options "-Xmx85g" \
 ...
 #gatk IndexFeatureFile \
     -F $BASE_DIR/outputs/6_genotyping/6_2_all/intermediate.vcf.gz \
...
echo -e "\${P}/\${PREFIX}_filterd.vcf.gz\n" >> $BASE_DIR/outputs/lof/17_pca.fofn
EOA
